...
uniform int celLevels = 5;//Number of shading levels
uniform float celSpecularCutoff = 0.3;//The cutoff point of the levels for specular
 
const float celScaleFactor = 1.0 / celLevels;//Scale for cel intensity

uniform float celOutline = 0.2;//Used to control outline threshold
...
void phong(in Light light, in vec3 position, in vec3 normal, out vec3 diffuse, out vec3 specular)
{
	...
	float cellIntensity = floor(intensity * celLevels) * celScaleFactor;//clamp the ranges and scale intensity
	diffuse = (light.color * cellIntensity);//set the diffuse lighting and modify by the celintensity
	...
	if(intensity > 0)
	{
		...
		intensity = (intensity < celSpecularCutoff) ? 0 : 1;//set cuttoff so the specular hilight matches
...
void main()
{
	...
	// Problem with emissive_texture_mask
